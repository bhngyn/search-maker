...
export function render(chip, handlers) {
  ...
  del.setAttribute('aria-label', 'حذف النطاق الرقمي');
  del.textContent = '×';
  ...
  lowInput.placeholder = 'من';
  lowInput.value = chip.props.low ?? '';
  lowInput.setAttribute('aria-label', 'الرقم الأدنى');
  ...
  highInput.placeholder = 'إلى';
  highInput.value = chip.props.high ?? '';
  highInput.setAttribute('aria-label', 'الرقم الأقصى');
  ...
  prefixInput.setAttribute('aria-label', 'البادئة');
