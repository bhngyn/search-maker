...
1. **Category bar** — a single segmented-button row (`الأعلى`, `المنشورات`, `الأشخاص`, `الصور`, `الفيديو`, `الصفحات`) styled to mirror the header's `.engine-toggle`. The active button gets the `--accent` solid fill + `aria-checked="true"`. No card chrome around the row — it sits flat above the keyword card. Each button carries an `sr-only` hint span via `aria-describedby` (the description that used to be a visible 2-line subtitle on the old card variant).
2. **Required keyword card** — the keyword is the only required input and gets its own visually distinct card (`--bg-subtle` background, 1px border, 10px radius). The card stacks vertically: a prominent label, an enlarged input (44px min-height, 16px font), a muted info hint, and a dismissable amber **boolean/quotes warning** that explains Facebook ignores `AND`/`OR` and `"…"` quotation marks. The warning is a `<button>` with click-to-dismiss; once dismissed it stays dismissed for the rest of the session (state lives in `state.keywordWarningDismissed` — a refresh restores it, consistent with the no-persistence rule).
3. **Per-category filter sections** — single bordered container holds all filter rows separated by 1px dividers (no per-section card). Each row is a CSS-grid `[legend | controls]` pair where the controls column is a horizontal `.fb-pill-row` of `<button class="fb-pill" role="radio" aria-checked>` (radio sections) or a single `<button aria-pressed>` toggle pill (Pages → الحساب الموثّق). Pill styling mirrors `.idiom-panel-group-chip` (28px tall, 999px radius, `--accent` solid pressed). Section composition: top → الترتيب + كاتب المنشور + نوع المنشور + ضمن مجموعة + تاريخ النشر; posts → same minus الترتيب; people → أصدقاء مشتركون only; photos → كاتب المنشور + نوع الصور + تاريخ النشر; videos → مصدر الفيديو + تاريخ النشر; pages → الحساب الموثّق + فئة الصفحة. Switching category resets sections/toggles/date but preserves the keyword and the warning-dismissed flag.
4. **Date range** — one row inside the sections container, label-leading + two side-by-side date inputs (Arabic Hijri/Gregorian custom calendar in AR; native HTML5 `<input type="date">` in EN). Wired to the `rp_creation_time` filter; the form emits Facebook's expected un-padded date strings (`2019-1-1`, not `2019-01-01`) and the triple-nested JSON.

**ID-driven filters are intentionally omitted.** Facebook's filter blob supports filtering by opaque numeric IDs (page ID, group ID, location ID, school ID, employer ID, user ID), but the form has no way to help an investigator discover those IDs. Surfacing the inputs without ID-discovery support produced empty / surprise-zero-results queries. The descriptor therefore drops:
- `peopleCity`, `peopleEducation`, `peopleWork` sections (each was a single ID-only field).
- `taggedLocation` section (its only non-default option required a location ID).
- `postsFrom → "From a Page"` option (kept the other 5 author scopes — me, friends, my groups & pages, public).
- `postedInGroup → "A specific group"` option (kept "بدون تحديد" + "مجموعاتي").
- `peopleMutual → "Friends of <user>"` option (kept "بدون تحديد", "أصدقائي", "أصدقاء أصدقائي").

If a future iteration adds a location-picker or page-search subform that resolves opaque IDs from human-readable names, those filters can be reintroduced.
...
3. **OSINT recipe library panel** — collapsible catalog of pre-built operator recipes (35 per chip-based engine). Default state is a single ~40px collapsed pill (`📚 وصفات بحث جاهزة · 35 وصفة · ▼`); clicking expands to a 5-column × 7-row grid of compact cards with a search input and group filter chips in the header bar. Clicking a card expands an inline inspector directly below it (full panel width) — chips are seeded only via an explicit Apply button, not on card-click. Visually distinct from the chip area (raised `--bg-subtle` surface, solid border, larger corner radius). Hidden when the active engine is Facebook. See **OSINT recipe library panel** section below.
...
**The panel must defer to the chip composer in vertical space, visual weight, and focus handoff.** The chip composer is the main interface; the idiom panel is a side reference. Concretely: the panel body keeps its existing `max-height: min(620px, 60vh)` cap and scrolls internally when an inspector is open; the panel surface (`--bg-subtle`) stays a half-step quieter than the chip area; every apply path fires the existing `.chip-section--just-seeded` outline-flash and calls `focusComposer()` so the user's eye is deliberately pulled down from the panel to the composer; the panel stays open after apply (investigators stack recipes) but the inspector collapses the just-applied card after ~700ms; the applied-indicator subscribes to `chipState.subscribe` so chip-array changes are reflected immediately; the "Build it manually" steps reference composer controls by label only — they are not clickable proxies.

**Three-axis state machine** driven by `data-state`, `data-descriptions`, and `data-open-id` attributes on the panel root; CSS handles all visual transitions, JS only mutates the attributes:
...
- **State 2 — open compact grid**: pill becomes a header bar (title + search input + group filter chips + `📖 شروح` toggle + `▲` close chevron); body shows seven Arabic section headers and 35 compact cards (5 per row, ~38px tall, icon + title only). Clicking a card opens its inline inspector (State 3 for that card). No hover popovers — the inspector is the preview surface.
- **State 3 — inspector open** (`data-open-id="<idiom-id>"`): an inspector row (`.idiom-panel-inspector`) is inserted immediately after the card's grid row, spanning the full panel width (~220px tall). Only one inspector is open at a time; clicking another card closes the current one and opens the new one. The `📖 شروح` toggle remains available for the power-scan "expand all cards to title + pattern + 1-line description" mode — this is orthogonal to the per-card inspector and the two surfaces do not duplicate signal (expanded-all shows `pattern`; inspector shows anatomy + how-to).

**Inspector contents** (top to bottom inside `.idiom-panel-inspector`):

1. **Localized title + applied indicator** — recipe title in the active language. If all non-empty anatomy chips are present in the chip array, a small `✓` glyph appears and the primary action button relabels to `أضِف مرة أخرى`.
2. **"What this recipe does"** (`idiom.section.whatItDoes`) — one-paragraph Arabic description of the recipe's investigative purpose.
3. **"Recipe anatomy"** (`idiom.section.anatomy`) — read-only chip previews rendered through the existing per-chip-type `render(chip, handlers)` functions with no-op handlers and a `chip--readonly` wrapper class. Each anatomy chip is paired with a small `+` button (`idiom.addThisChip`) that adds only that one chip to the workspace (advanced affordance). OR connectors render as grouped OR chains identical to the real chip area.
4. **"Build it manually"** (`idiom.section.howto`) — numbered list of Arabic step strings. Each step references a composer/drawer control by its i18n-resolved label wrapped in a `.idiom-control-mention` mini-pill (same blue accent + border-radius + monospace font as the real composer pill — a visual rhyme, not a clickable proxy). Generated by `src/idioms/explain.js` from the captured anatomy array.
5. **"Assembled query"** (`idiom.section.assembled`) — the recipe's `pattern` string in a `<code dir="ltr">` block.
6. **Action buttons** — `أضِف الوصفة كاملة` (primary, `idiom.applyRecipe`) and `استبدل البحث الحالي` (secondary, `idiom.replaceRecipe`). Replace clears the chip array before applying.

**Anatomy capture** uses a sandboxed chip-state mock (`src/idioms/sandbox.js` — `captureAnatomy(idiom)`): a fake chip-state that records `add()` / `addAfter()` calls in order without notifying anything, wrapped in try/catch so a misbehaving idiom degrades to rendering just the `pattern` string with an `idiom.anatomy.unavailable` notice. Results are memoised per `(idiom.id, engine.id, lang.get())` and cleared on engine swap.

**"Build it manually" steps** are generated by `src/idioms/explain.js` (`explainAnatomy(captured, engine, lang)`): a pure mapper from the captured-anatomy array to localised step strings. Keyword chips emit `idiom.howto.keyword` (with pill-click step) or `idiom.howto.keyword.plain` (plain word, no pill step) or `idiom.howto.keyword.empty` (placeholder text). Negate/quote states append `idiom.howto.negate` / `idiom.howto.quote` sub-steps. OR-connector chips emit `idiom.howto.or`. Special chip types (filetype, date-range, proximity, number-range, filter, engagement) emit `idiom.howto.special` with the drawer item label resolved from the engine descriptor.

**Search + group filter** (open state header): the search input filters cards by `title.{ar,en}` + `description.{ar,en}` + `pattern` substring (case-insensitive); group filter chips narrow visible groups (multi-select). Empty groups are hidden when a filter is active but their header row is shown so the user understands the structure. Pressing `/` focuses the search input when the panel is open and focus is not already in the composer or a chip. Esc with search focused clears the search; Esc with inspector open closes the inspector; Esc with panel open closes the panel — never touches the composer.

**Keyboard**: Tab cycles through search, group chips, and cards. ↑/↓ on a focused card moves focus within the grid. Enter on a closed card opens the inspector. Enter while focus is inside an open inspector (on the Apply button) seeds the chips. Double-click a card skips the inspector and applies directly. No global Enter/Space bindings.

**Click flow**: clicking the Apply button runs `idiom.apply(chipState)` directly (equivalent to the legacy `applyTemplate` path); preview fragments flash for ~600ms; the chip section gains `.chip-section--just-seeded` outline-flash for ~400ms; `focusComposer()` is called. The inspector auto-collapses the card after ~700ms. **The panel does NOT auto-collapse** — investigators stack recipes. The user explicitly clicks `▲` when done.

**Already-applied indicator**: `chipState.subscribe` triggers a recompute after every chip mutation. A recipe is "applied" if every non-empty anatomy chip has a matching chip in the current chip array, matched on `(type, operator, negate, quoted, non-empty text)`. Empty placeholder chips (`text: ''`) are excluded from the match — this prevents false positives when the workspace is empty.

**Engine swap reset**: the panel collapses to State 1 (`data-state="closed"`, `data-open-id=""`) on every engine change. The chip array is preserved across Google ↔ X swaps; only the panel resets.
...
The engine toggle in the header switches between Google, X / Twitter, and Facebook. Switching Google ↔ X preserves the chip array. Switching to Facebook hides the chip section, welcome panel, warnings region, and tips region; the Facebook form replaces them. The preview box shows the assembled `https://www.facebook.com/search/{category}/?q=…&epa=FILTERS&filters={base64}` URL on every form change. The Facebook form's category bar switches the visible filter sections; switching categories resets the within-category filter selections (since section lists vary). The required keyword field is passed verbatim (URL-encoded only). Combining "منشورات أصدقائي" (Posts From → friends) with "منشورات شاهدتُها" (Post Type → interacted) produces a `filters=` parameter whose base64 decodes to the WhoPostedWhat reference's exact JSON: `{"rp_author":"{\"name\":\"author_friends_feed\",\"args\":\"\"}","interacted_posts":"{\"name\":\"interacted_posts\",\"args\":\"\"}"}`. The "تاريخ النشر" range emits Facebook's nested-args form with un-padded month/day strings (`2019-1-1`, not `2019-01-01`). ID-driven filters (page-ID, group-ID, location-ID, city/school/employer/user-ID) are not exposed by the form — see "Form layout" above for the rationale and the dropped sections/options.
...
The mode toggle is visible in the header at all times in both modes. Switching from Beginner to Advanced preserves the chip array; switching back also preserves them. In Beginner mode the welcome panel (collapsing it removes the row entirely; refresh restores it), the empty-state templates picker, the ghost-chip preview, the operator-conversion pills, and the composer hint are visible. In Advanced mode all of those are hidden, and three additional interactions become available: drag-to-reorder, Alt+Arrow keyboard reorder, and Shift+Click multi-select with the bulk-actions toolbar.
...
src/
  ...
  idioms/
    ...
    sandbox.js            captureAnatomy(idiom) — sandboxed chip-state mock that records add/addAfter calls without side effects; memoised per (idiom.id, engine.id, lang)
    explain.js            explainAnatomy(captured, engine, lang) → string[] — pure mapper from captured-anatomy array to localised "Build it manually" step strings
  chips/
  ...
  ui/
    ...
    idiom-panel.js        recipe library panel: three-axis state machine (data-state × data-descriptions × data-open-id), inline inspector per card, search + group-filter, applied-state subscriber, keyboard nav, Apply / Replace / per-chip-+ flows; imports captureAnatomy and explainAnatomy
...
- The empty-state templates picker that used to render inside the chip area when the chip array was length 0 has been retired in favor of a dedicated **OSINT recipe library panel** that sits as its own region above the chip section. The panel carries 35 idioms per chip-based engine (Google + X), opens/closes independently of chip-array state, and remains accessible mid-composition so investigators can stack recipes (`intitle:` + `filetype:pdf` from two clicks). The panel is hidden when Facebook is active. New files: `src/idioms/google.js`, `src/idioms/x.js`, `src/ui/idiom-panel.js`, `src/styles/idioms.css`. Engine descriptors expose the idiom catalog as `idioms` / `idiomGroupOrder` / `idiomGroupLabels` alongside the legacy `templates` field; the panel reads the new fields, the legacy `templates` is no longer consumed by any UI surface but is kept on the descriptor for any future code that wants the simpler 3-card list. The chip-area's empty state is now a single-line muted hint paragraph (`أو اكتب كلمة في الأسفل وابدأ من الصفر.`) that bridges the recipe panel above with the composer below. See **OSINT recipe library panel** section for the full UX spec.
- The OSINT recipe library panel was rebuilt as an investigator's playbook with a click-to-inspect model: clicking a card expands an inline inspector that describes the recipe, shows its chips as read-only anatomy previews, lists numbered "Build it manually" steps referencing real composer controls by label (wrapped in `.idiom-control-mention` mini-pills as a visual rhyme), and shows the assembled `pattern` string. Applying chips is now an explicit button click (`أضِف الوصفة كاملة`) rather than a side-effect of clicking the card itself. The panel gains a search input and group filter chips in its header bar, and a per-recipe already-applied indicator that subscribes to `chipState.subscribe`. Anatomy is captured by running `idiom.apply()` against a sandboxed chip-state mock (`src/idioms/sandbox.js`) and rendered through the existing per-chip `render()` functions with no-op handlers and a `chip--readonly` class. Step strings are generated by `src/idioms/explain.js`. The previous hover preview band and the never-shipped `src/ui/idiom-popover.js` reference are removed.
- The Facebook form was rewritten for compactness and to drop ID-driven filters. Earlier the form stacked ~7 bordered cards vertically (categories card → keyword card → bordered filter sections each containing a vertical radio list), forcing significant scrolling to reach the URL preview at the bottom. The current shape is: a flat segmented `.fb-cat-grid` category bar (mirrors the header's `.engine-toggle`), a distinct keyword card on `--bg-subtle` with an enlarged input + muted hint + dismissable amber boolean/quotes warning (`state.keywordWarningDismissed` persists the dismiss across category switches within the session), and a single bordered `.fb-form-sections` container that holds all filter rows separated by 1px dividers. Each filter section is a CSS-grid `[legend | controls]` row where the controls are a horizontal `.fb-pill-row` of `<button class="fb-pill" role="radio" aria-checked>` (radio sections) or one `<button aria-pressed>` toggle pill (Pages → الحساب الموثّق). Pill sizing/colors match `.idiom-panel-group-chip`. ID-driven filters were removed wholesale: the `peopleCity / peopleEducation / peopleWork` (idOnly) sections, the `taggedLocation` section, and the ID-bearing options `postsFrom → author_page`, `postedInGroup → group`, `peopleMutual → friends_of` are all dropped from `src/engines/facebook.js`. Their i18n keys are still present in `messages.js` but no longer referenced — kept for now to avoid churn, can be pruned later. Render functions for the obsolete cases (`renderIdOnlySection`, the `idField` rendering branch, `state.ids`) are removed from `src/ui/facebook-form.js`. The form's vertical extent on the "top" category at desktop widths dropped from roughly 600px+ (old card stack) to ~470px (compact rows + keyword card), and on "people" from ~430px (4 idOnly cards) to ~150px (single pill row). The `body.engine-facebook` show/hide rules and the URL-assembly logic in `buildFacebookUrl` are unchanged.
